...
```{r Pivoting the data + New binary column}
...
```{r Group analysis: Pivoting the data}
...
```{r Group analysis: nesting the data}
...
```{r Group analysis: Grouping by gene}
...
```{r Group analysis: Top 10 candidate genes}
...
```{r Group analysis: Saving the results}
...
```{r Group analysis: Saving the ordered results}
...
volcano <- ggplot(borovecki_estimates, aes(x = estimate, y = -log10(p.value))) + 
  ...
  geom_text_repel(aes(color=is_significant, label = top10), size = 3, max.overlaps = Inf) +
  labs(
    title = "Genes Associated with hungington disease",
