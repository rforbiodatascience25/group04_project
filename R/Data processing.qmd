...
```{r Histogram of raw expression values}
#| message = FALSE
...
```{r Getting min and max values of raw data}
borovecki_tibble |>
...
```{r Getting min and max values after log2 transformation}
borovecki_data |>
  summarise(min_exp = min(log2_expression),
            max_exp = max(log2_expression))
```

```{r Comparison between raw data and log2 data. Histogram}
#| message = FALSE
...
```{r Log2-transformed expression values. Histogram}
#| message = FALSE
borovecki_data |> 
...
```{r Comparison between raw data and log2 data. Boxplot}
#| message = FALSE
...
```{r Grouping according expression level}
...
```{r Get total count in each expression group and percent}
borovecki_grouped |> 
  count(expressionClass) |> 
...
```{r Boxplot comparison of raw data and log2 expression}
...
```{r Most expressed gene}

borovecki_tibble |>
  slice_max(expressionValue, n = 1)
```

```{r Less expressed gene}

borovecki_tibble |>
  slice_min(expressionValue, n = 1)
```

```{r Boxplot o the most expressed gene}
borovecki_grouped |>
  filter(gene == "HBA1") |>
...
```{r Boxplot of the less expressed gene}
borovecki_grouped |>
  filter(gene == "ZNF675") |>
  ggplot(aes(x = gene, y = log2_expression, color = diagnosis)) +
  geom_boxplot() +
...
```{r Boxplot of the most expressed genes found in the library}
...
geneExpression_comparison_boxplot
ggsave(
...
This plot compares the expression levels of several genes that have been previously associated with Huntington's disease according to the literature. We wanted to evaluate if this is also reflected in our data set by examining differences in gene expression values between control individuals and symptomatic individuals. The plot allows us to visually assess whether symptomatic individuals show higher or lower gene expression levels compared to control individuals. Overall, we can observe a shift in expression values between control and symptomatic individuals, which matches what found in literature.
...
```{r Violin plot of HTT gene expression}
...
```{r Violen Plot of PMS1 gene expression}
...
```{r Violin Plot of ZDHHC17 genes expression}
...
For ZDHHC17, there is a higher gene expression for symptomatic patients compared to control patients. ZDHHC17 is a HTT-interacting protein that plays a key role in palmitoylation of neuronal proteins.
...
```{r Boxplot of more expressed gene after log2}
borovecki_data |>
  filter(gene == "HBA1") |>
...
```{r Bosplot of less expressed gene after log2}
borovecki_data |>
  filter(gene == "ZNF675") |>
  ggplot(aes(x = gene, y = log2_expression)) +
  geom_boxplot() +
...
```{r DNA Repair genes expression level boxplot}
#| message = FALSE
...
Expression_of_DNA_repair_genes_vs_clinical_outcome <- ggplot(dnarepair_data, aes(x = log2_expression, 
  ...
  labs(
    ...
    caption="Data from: Borovecki et al. 2005\nData from: Namuli et al. 2024"
  ) +
  theme_minimal() 

Expression_of_DNA_repair_genes_vs_clinical_outcome
ggsave(
...
```{r Comparison of HBA1 vs ZNF675 Expression }
...
```{r Comparison of HTT vs LIG1 Expression}
...
```{r Comparison_of_HTT_vs_MHL1_Expression}
#| message = FALSE
...
```{r Comparison_of_HTT_vs_PMS1_Expression}
#| message = FALSE
...
```{r Comparison of HTT vs Repair gene Expression}
...
Namuli, K. L., Slike, A. N., Hollebeke, M. A., & Wright, G. E. B. (2024). Genomic characterization of Huntington’s disease genetic modifiers informs drug target tractability. *Brain Communications*, *7*(1), fcae418. https://doi.org/10.1093/braincomms/fcae418

Borovecki, F., Lovrecic, L., Zhou, J., Jeong, H., Then, F., Rosas, H. D., Hersch, S. M., Hogarth, P., Bouzou, B., Jensen, R. V., & Krainc, D. (2005). Genome-wide expression profiling of human blood reveals biomarkers for Huntington’s disease. *Proceedings of the National Academy of Sciences*, *102*(31), 11023–11028. https://doi.org/10.1073/pnas.0504921102
